...
class FletCalendar(ft.UserControl):
    ...
    def set_theme(self, border_color=ft.colors.BLUE_500, 
                  text_color=ft.colors.PINK_50, 
                  current_day_color=ft.colors.RED_500):
    ...
    def build(self):
        ...
        # Add Spanish day names above each day in the calendar
        day_names_row = ft.Row()
        for day_name in spanish_days:
            day_name_label = ft.Text(day_name, size=12, color=self.text_color)
            day_names_row.controls.append(ft.Container(day_name_label, width=40, height=20, alignment=ft.alignment.center))
        
        calendar_column.controls.append(day_names_row)
